...
`guji` 按 `config.yaml` 里各 provider 的 `launch:` 块，在真正需要时（`guji ask`/
`guji search`/`guji eval`）自动拉起 `ollama serve`、本地 LLM 的 `llama-server`、
reranker 的 `llama-server` 这三个进程，命令跑完就停掉自己启动的那个；如果对应
`base_url` 已经有服务在监听，guji 不会碰它。不需要手动起任何服务。日常使用三步：
装依赖、下模型/数据、问问题。
...
```bash
guji models fetch                              # 下载本地 LLM + reranker 的 GGUF（profile: local 需要）
uv sync --extra release                                      # release fetch 需要，日常问答不需要
guji release fetch bajzc/guji-rag-data --label 20260801 --force # 下载预建索引，跳过语料处理/建索引（见文末「分发预建数据」）
```

embedding 模型（Ollama，检索必需，与 profile 无关）由 `ollama serve` 按需自动拉起，
首次用时自动 `ollama pull qwen3-embedding:0.6b`。

若 `profile: cloud`，跳过 `guji models fetch` 里的 LLM，改为在 `.env` 中填
`DASHSCOPE_API_KEY`（cloud LLM 走 `qwen-max`；默认配置下 rerank 也走这把 key）：

```bash
cp .env.example .env   # 填入 DASHSCOPE_API_KEY
```

不想依赖 DashScope 也不想跑本地 reranker：`ask`/`search` 加 `--no-rerank`（检索质量
会下降，RRF 排序直接生效）。
...
guji ask "..." --no-rerank                                 # 跳过 rerank，省 DashScope key / 本地 reranker
...
本地 LLM + reranker 各是一个常驻 `llama-server`，加上常驻的 embedding（Ollama），
三者同时跑很容易在 16G 统一内存机器上 OOM。`config.yaml` 里 `providers.local.llm` /
`rerank` / `embedding` 各自的 `launch:` 块（默认已配好，见 `config.yaml`）就是为此
存在的：guji 只在命令真正用到某个后端时才拉起对应进程，跑完就把自己启动的那个停掉；
如果 `base_url` 已经有服务在监听（比如你手动起的），guji 不会碰它，也不会去停它。
日志写在 `.guji/logs/`。
...
guji 按需拉起的 `llama-server` 命令行来自 `config.yaml: providers.local.llm.launch.command`
（`rerank.launch.command` 同理，见「快速开始」）；macOS 版本与 Linux 版本的差异：
...
`llama-server` 命令行同样来自 `config.yaml: providers.local.llm.launch.command`（GPU 卸载 +
MoE 专家 CPU 卸载，`--n-cpu-moe` 按显存占用率现场调，见下方 `--n-cpu-moe 26` 的调参说明）。
